      ...
      <div className="min-h-screen grid-bg flex items-center justify-center p-4">
        <div className="w-full max-w-sm">
          <div className="text-center mb-8">
            ...
            <p className="font-mono text-xs text-muted mt-2 tracking-wider">RESTRICTED AREA</p>
          </div>
          <div className="bg-card border border-border rounded-2xl p-8">
            <form onSubmit={handleLogin} className="space-y-4">
              <div>
                <label className="font-mono text-xs text-muted block mb-2 tracking-wider">USERNAME</label>
                <input
                  type="text"
                  value={username}
              ...
              {loginError && (
                <p className="font-mono text-xs text-warn bg-warn/10 border border-warn/20 rounded-lg px-3 py-2">{loginError}</p>
              )}
              <button
                type="submit"
            ...
            <Link to="/" className="block text-center font-mono text-xs text-muted hover:text-white transition-colors mt-4">
              ← Back to Terminal
            </Link>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen grid-bg p-4 md:p-8">
      ...
      {selectedImg && (
        ...
        >
          <img src={selectedImg} alt="capture" className="max-w-sm max-h-96 rounded-xl object-cover" />
        </div>
      )}
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          <StatCard label="Total Records" value={records.length} color="text-white" />
        ...
        <div className="mb-4 flex items-center gap-2 bg-accent/5 border border-accent/20 rounded-lg px-4 py-2">
          ...
          <p className="font-mono text-xs text-muted">Showing records from the last 48 hours. Download the sheet daily — records are auto-deleted after 48h.</p>
        </div>
          ...
          {loading ? (
            ...
            <div className="flex items-center justify-center py-20">
              <div className="text-center">
                ...
                <p className="font-mono text-sm text-muted">No records in the last 48 hours</p>
              </div>
            </div>
          ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border">
                    <th className="text-left font-mono text-xs text-muted tracking-wider px-6 py-4">EMPLOYEE</th>
                    <th className="text-left font-mono text-xs text-muted tracking-wider px-6 py-4">ACTION</th>
                    <th className="text-left font-mono text-xs text-muted tracking-wider px-6 py-4">TIME</th>
                    <th className="text-left font-mono text-xs text-muted tracking-wider px-6 py-4">DATE</th>
                    <th className="text-left font-mono text-xs text-muted tracking-wider px-6 py-4">PHOTO</th>
                  </tr>
                </thead>
                <tbody>
                  {records.map((r, i) => {
                    ...
                    return (
                      ...
                      >
                        <td className="px-6 py-4">
                          <span className="font-sans text-sm text-white font-medium">{r.name}</span>
                        </td>
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full font-mono text-xs font-bold ${
                            r.type === 'checkin'
                        ...
                        <td className="px-6 py-4">
                          {r.image_url ? (
                            ...
                            >
                              <img src={r.image_url} alt="cap" className="w-full h-full object-cover" />
                            </button>
                          ) : (
...
function StatCard({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div className="bg-card border border-border rounded-xl px-6 py-5">
      <p className="font-mono text-xs text-muted tracking-wider mb-2">{label.toUpperCase()}</p>
